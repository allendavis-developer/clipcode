...
function naturalOf(dir, rel) {
  try {
    ...
    const m = s.match(/\bduration\s*\(\s*(\d+(?:\.\d+)?)/)
...
/* Empty. duration() is optional now — a clip is as long as its choreography
   unless it says otherwise — so there is nothing a new clip has to start with
   and nothing to delete before you begin. */
export const BLANK = () => '';
